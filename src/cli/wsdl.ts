...
export const getComplexContentNode = (node: XmlNode): XmlNode | undefined => {
...
export const getExtensionNode = (node: XmlNode): XmlNode | undefined => {
    const extensionField = Object.keys(node).find(objectField => objectField.match(/([a-zA-z0-9]*:)?extension/));
    return node[extensionField!];
};

export const getRestrictionNode = (node: XmlNode): XmlNode | undefined => {
...
export const complexTypeToObject = (node: XmlNode | XmlNode[], namespaces: Map<string, string>, complexTypes: ComplexTypes, targetNamespace: string | undefined): any => {
    if (Array.isArray(node)) {
...
export const schemaToObject = (node: XmlNode, namespaces: Map<string, string>, complexTypes: ComplexTypes): any => {
    ...
    if (Array.isArray(elementNode)) {
        ...
        const complexTypeNode = getComplexTypeNode(node);
        object[(elementNode as XmlNode).type ?? (elementNode as XmlNode).name!] = complexTypeToObject(complexTypeNode!, namespaces, complexTypes, node.targetNamespace);
    }
